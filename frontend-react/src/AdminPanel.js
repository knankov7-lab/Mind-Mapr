  ...
  return (
    <div style={{position:'absolute',right:18,top:62,bottom:14,left:380,background:'rgba(18,26,46,.92)',padding:24,borderRadius:'18px',boxShadow:'0 18px 55px rgba(0,0,0,.35)',overflow:'auto',minWidth:520}}>
      ...
      <div style={{display:'flex',gap:8,marginBottom:16,flexWrap:'wrap'}}>
        ...
        <button className={tab==='saves'?'btn primary':'btn ghost'} onClick={()=>setTab('saves')}>Записи</button>
      ...
      {tab==='settings' && (
        <section>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,fontSize:13}}>
            ...
            <label style={{background:'rgba(255,255,255,.04)',padding:10,borderRadius:8,border:'1px solid rgba(255,255,255,.10)'}}>
              <div style={{fontWeight:600,marginBottom:4,color:'#dfe6ff'}}>🔧 Режим на поддръжка</div>
              <select value={settings.maintenanceMode?'on':'off'} onChange={e=>setSettings({...settings, maintenanceMode: e.target.value==='on'})} 
                style={{width:'100%',padding:6,borderRadius:6,border:'1px solid rgba(255,255,255,.12)',background:'rgba(255,255,255,.07)',color:'#e9eeff',cursor:'pointer'}}>
                <option value="off">Изключен</option>
                <option value="on">Включен</option>
              </select>
              <p style={{fontSize:11,opacity:.7,margin:'4px 0 0 0'}}>Само админи могат да влизат</p>
            </label>

            <label style={{background:'rgba(255,255,255,.04)',padding:10,borderRadius:8,border:'1px solid rgba(255,255,255,.10)'}}>
              <div style={{fontWeight:600,marginBottom:4,color:'#dfe6ff'}}>📊 Макс. записи на потребител</div>
              <input type="number" min="1" max="1000" value={settings.maxSavesPerUser||100} onChange={e=>setSettings({...settings, maxSavesPerUser: parseInt(e.target.value)||100})} 
                style={{width:'100%',padding:6,borderRadius:6,border:'1px solid rgba(255,255,255,.12)',background:'rgba(255,255,255,.07)',color:'#e9eeff'}} />
              <p style={{fontSize:11,opacity:.7,margin:'4px 0 0 0'}}>Версии на карта</p>
            </label>

            <label style={{background:'rgba(255,255,255,.04)',padding:10,borderRadius:8,border:'1px solid rgba(255,255,255,.10)'}}>
              <div style={{fontWeight:600,marginBottom:4,color:'#dfe6ff'}}>👥 Макс. участници в стая</div>
              <input type="number" min="1" max="500" value={settings.maxRoomUsers||50} onChange={e=>setSettings({...settings, maxRoomUsers: parseInt(e.target.value)||50})} 
                style={{width:'100%',padding:6,borderRadius:6,border:'1px solid rgba(255,255,255,.12)',background:'rgba(255,255,255,.07)',color:'#e9eeff'}} />
              <p style={{fontSize:11,opacity:.7,margin:'4px 0 0 0'}}>Едновременни потребители</p>
            </label>

            <label style={{background:'rgba(255,255,255,.04)',padding:10,borderRadius:8,border:'1px solid rgba(255,255,255,.10)'}}>
              <div style={{fontWeight:600,marginBottom:4,color:'#dfe6ff'}}>⏰ Изтичане на сесия (мин.)</div>
              <input type="number" min="5" max="10080" value={settings.sessionTimeout||480} onChange={e=>setSettings({...settings, sessionTimeout: parseInt(e.target.value)||480})} 
                style={{width:'100%',padding:6,borderRadius:6,border:'1px solid rgba(255,255,255,.12)',background:'rgba(255,255,255,.07)',color:'#e9eeff'}} />
              <p style={{fontSize:11,opacity:.7,margin:'4px 0 0 0'}}>480 = 8 часа</p>
            </label>

            <label style={{background:'rgba(255,255,255,.04)',padding:10,borderRadius:8,border:'1px solid rgba(255,255,255,.10)'}}>
              <div style={{fontWeight:600,marginBottom:4,color:'#dfe6ff'}}>✅ Публични карти</div>
              <select value={settings.publicMapsApproval?'manual':'auto'} onChange={e=>setSettings({...settings, publicMapsApproval: e.target.value==='manual'})} 
                style={{width:'100%',padding:6,borderRadius:6,border:'1px solid rgba(255,255,255,.12)',background:'rgba(255,255,255,.07)',color:'#e9eeff',cursor:'pointer'}}>
                <option value="auto">Автоматично</option>
                <option value="manual">Ръчно одобрение</option>
              </select>
              <p style={{fontSize:11,opacity:.7,margin:'4px 0 0 0'}}>За публичния списък</p>
            </label>

            <label style={{background:'rgba(255,255,255,.04)',padding:10,borderRadius:8,border:'1px solid rgba(255,255,255,.10)'}}>
              <div style={{fontWeight:600,marginBottom:4,color:'#dfe6ff'}}>📏 Макс. възли на карта</div>
              <input type="number" min="10" max="10000" value={settings.maxNodesPerMap||1000} onChange={e=>setSettings({...settings, maxNodesPerMap: parseInt(e.target.value)||1000})} 
                style={{width:'100%',padding:6,borderRadius:6,border:'1px solid rgba(255,255,255,.12)',background:'rgba(255,255,255,.07)',color:'#e9eeff'}} />
              <p style={{fontSize:11,opacity:.7,margin:'4px 0 0 0'}}>Ограничение</p>
            </label>

            <label style={{background:'rgba(255,255,255,.04)',padding:10,borderRadius:8,border:'1px solid rgba(255,255,255,.10)'}}>
              <div style={{fontWeight:600,marginBottom:4,color:'#dfe6ff'}}>📝 Логване</div>
              <select value={settings.enableLogging?'on':'off'} onChange={e=>setSettings({...settings, enableLogging: e.target.value==='on'})} 
                style={{width:'100%',padding:6,borderRadius:6,border:'1px solid rgba(255,255,255,.12)',background:'rgba(255,255,255,.07)',color:'#e9eeff',cursor:'pointer'}}>
                <option value="on">Включено</option>
                <option value="off">Изключено</option>
              </select>
              <p style={{fontSize:11,opacity:.7,margin:'4px 0 0 0'}}>Запис на действия</p>
            </label>

            <label style={{background:'rgba(255,255,255,.04)',padding:10,borderRadius:8,border:'1px solid rgba(255,255,255,.10)'}}>
              <div style={{fontWeight:600,marginBottom:4,color:'#dfe6ff'}}>🎨 Тема</div>
              <select value={settings.theme||'dark'} onChange={e=>setSettings({...settings, theme: e.target.value})} 
                style={{width:'100%',padding:6,borderRadius:6,border:'1px solid rgba(255,255,255,.12)',background:'rgba(255,255,255,.07)',color:'#e9eeff',cursor:'pointer'}}>
                <option value="dark">Тъмна</option>
                <option value="light">Светла</option>
              </select>
              <p style={{fontSize:11,opacity:.7,margin:'4px 0 0 0'}}>За всички</p>
            </label>

            <label style={{background:'rgba(255,255,255,.04)',padding:10,borderRadius:8,border:'1px solid rgba(255,255,255,.10)'}}>
              <div style={{fontWeight:600,marginBottom:4,color:'#dfe6ff'}}>🌐 Язык</div>
              <select value={settings.lang||'bg'} onChange={e=>setSettings({...settings, lang: e.target.value})} 
                style={{width:'100%',padding:6,borderRadius:6,border:'1px solid rgba(255,255,255,.12)',background:'rgba(255,255,255,.07)',color:'#e9eeff',cursor:'pointer'}}>
                <option value="bg">Български</option>
                <option value="en">English</option>
                <option value="ru">Русский</option>
              </select>
              <p style={{fontSize:11,opacity:.7,margin:'4px 0 0 0'}}>По подразумеване</p>
            </label>
          </div>

          <div style={{marginTop:16,display:'flex',gap:8,flexWrap:'wrap'}}>
            <button className="btn primary" onClick={saveSettings} disabled={saving} style={{fontSize:13,padding:'10px 16px'}}>
              {saving? '⏳ Запазване...' : '💾 Запази настройки'}
            </button>
            <button className="btn ghost" onClick={fetchAll} style={{fontSize:13,padding:'10px 16px'}}>🔄 Презареди</button>
          </div>
          
          <div style={{marginTop:12,padding:10,background:'rgba(124,92,255,.12)',borderRadius:8,fontSize:12,opacity:.85,border:'1px solid rgba(124,92,255,.20)'}}>
            ⓘ Всички промени се запазват в БД. Ако има проблеми, презареди.
          </div>
        </section>
      )}
    </div>
